...
const Attendance: React.FC = () => {
  ...
  // Filter months to show from previous month onwards (if previous month is in current year)
  const startMonth = currentMonth > 0 ? currentMonth - 1 : currentMonth; // Include previous month if not January
  const availableMonths = monthNames.slice(startMonth).map((month, index) => {
    const monthNumber = startMonth + index + 1; // Convert to 1-based month number
