...
const Reports: React.FC = () => {
  ...
  const fetchDepartments = async () => {
    try {
      ...
      const uniqueDepartments = [...new Set(employees.data.map((emp: any) => emp.department))]
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        img.onload = () => {
          try {
            // Add logo
            pdf.addImage(img, 'JPEG', 15, 10, 30, 30);
            
            // Add company name and title next to logo
            pdf.setFontSize(20);
            pdf.setTextColor(37, 99, 235); // Blue color
            pdf.setFont('helvetica', 'bold');
            pdf.text('Dairy Licious', 50, 20);
            
            pdf.setFontSize(12);
            pdf.setTextColor(107, 114, 128); // Gray color
            pdf.setFont('helvetica', 'normal');
            pdf.text('HR Manager', 50, 28);
            
            pdf.setFontSize(14);
            pdf.setTextColor(0, 0, 0);
            pdf.setFont('helvetica', 'normal');
            pdf.text('HR Management System', 50, 35);
        ...
        img.onerror = () => {
          // If logo fails to load, add header without logo
          pdf.setFontSize(20);
          pdf.setTextColor(37, 99, 235);
          pdf.setFont('helvetica', 'bold');
          pdf.text('Dairy Licious', pageWidth / 2, 20, { align: 'center' });
          
          pdf.setFontSize(12);
          pdf.setTextColor(107, 114, 128);
          pdf.setFont('helvetica', 'normal');
          pdf.text('HR Management System', pageWidth / 2, 28, { align: 'center' });
      ...
      pdf.text(reportTitle, pageWidth / 2, 48, { align: 'center' });
      
      // Add metadata
      pdf.setFontSize(10);
      pdf.setTextColor(107, 114, 128);
      pdf.setFont('helvetica', 'normal');
      pdf.text(`Department: ${selectedDepartment || 'All Departments'}`, 20, 58);
      pdf.text(`Generated on: ${new Date().toLocaleDateString()}`, 20, 63);
      pdf.text(`Total Records: ${reportData.length}`, 20, 68);
